...
4. **Supply the whole line.** Use longitude/latitude `LineString` or explained `MultiLineString`, including the intended route/stage endpoints and no fabricated straight-line gaps. A real ferry/cable discontinuity gets a separate connection; missing trail geometry does not. A generated drive, walking, or hiking connector must never substitute for, repair, or overwrite established route geometry.
...
6. **Model access separately.** Add each trailhead, parking/transfer point, station, pier, or pickup as an `AccessPoint` with explicit modes, legal state, stopping/parking facts, source assertion, quality, and sensitivity. Link IDs with `accessPointIds`. Default law, parking, and hours to unknown without current competent evidence. Mixed-mode use requires current verified endpoints and current competent evidence that the intended access and stopping/parking use is legal; an unknown, stale, inferred, or merely nearby endpoint fails the gate.

7. **Model movement separately.** Add bus, rail, ferry, boat, gondola, funicular, cog railway, cable car, or pickup as `TransportConnection`, with current verified endpoint IDs, direction, freshness/validity, and its own assertions. Link through `transportConnectionIds`. A timed leg additionally requires an exact IANA time-zone identifier, applicable service calendar/date, sourced schedule, and exact usable departure times. Unknown, stale, timezone-ambiguous, or frequency-only service must not be treated as a usable departure.
...
9. **Add itinerary fixtures.** Test loop/out-and-back/point-to-point behavior as applicable, explicit walking connectors between every drive, access, transport, and hike endpoint, legal parking selection, exact timed departures, transport return or a verified different pickup, last-departure/closure refusal, and return-endpoint/stranding checks. Also assert source references, current verified route geometry, unchanged established geometry, no geometry substitution, and non-navigation-grade labeling. Refuse the itinerary when any required connector, endpoint, legal fact, departure, or return path fails its gate.
