"""Spot-check Wikimedia Commons hero photo URLs for Italian POIs.

The italy-pois.js dataset's `bp` URLs were generated by parallel research
agents. Although each agent verified URLs via web_fetch, hallucination is
still possible. This script HEAD-checks every URL and reports failures.

Usage:
    python tools/verify_poi_photos.py [--country IT|CH|FR|AT|all] [--limit N]
